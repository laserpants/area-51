...
programDefs = gets snd <#> yy123 . unpack

--yy123 :: Map (ProgKey (Type Int g)) (Definition MonoType (Expr t a0 a1 a2)) -> [Label (Type Int g)]
yy123 p =
  Map.toList p
    <#> \case 
      (Left name, def) -> (typeOf def, name)
      (Right label, _) -> label
...
liftDef name vs args expr = do
  insertDef (toProgKey name t ts) def
...
compile :: (MonadState (Int, Program MonoType Ast) m) => TypedExpr -> m Ast
compile =
  cata $ \case
