...
rowSubstitute :: Map Int (Type Int s) -> Row (Type Int t) Int -> Row (Type Int s) Int
rowSubstitute sub =
  cata $ 
    \case
      RNil -> rNil
...
instance (Substitutable a0, Substitutable a2) => Substitutable (Expr MonoType a0 a1 a2) 
  where
    apply sub =
      cata $ 
        \case
...
boundVars :: Scheme -> Set Name
boundVars = bound . unpack where
  bound = 
    cata 
      ( \case
