...
--      Just (Op t, op) | "f" == fun && arity t == length args ->
--        call op (zip as (repeat []))
--
--      Just (Op t, op) | "$lam2" == fun -> do
--        let sty = StructureType False (i8 : llvmType t : (LLVM.typeOf <$> as))
--        s <- malloc sty
--        p0 <- gep s [int32 0, int32 0]
--        p1 <- gep s [int32 0, int32 1]
--        store p0 0 (int8 1)  -- TODO
...
--        pure s
--
--      Just (Op t, op) | "$var_g_1" == fun -> do
--        undefined
--
--      _ ->
--        emitPrim (PInt 1)

--      Just (t, Op op) | "f" == fun && arity t == length args ->
--        Op <$> call op (zip (toOp <$> as) (repeat []))
--
--      Just (t, Op op) | "$lam2" == fun ->
--        pure (Partial (t, op) as)
--
--      Just (t, Partial (t1, op) bs) 
--        | arity t == length args ->
--          Op <$> call op (zip (toOp <$> (bs <> as)) (repeat []))
--        | otherwise -> do
--          let ttt = unwindType t1
--          pure (Partial (undefined, op) (bs <> as))
--
--      Just (t, Op op) ->
--        -- TODO
--        Op <$> emitPrim (PInt 1)
...
--        _ -> do
--          zz <- ask
--          traceShowM "___________________"
--          let Environment qq1 = zz in mapM_ traceShowM (Map.toList qq1)
--          traceShowM zz
--          error (show name)
--    ECall () (_, fun) args -> do
--      emitCall fun args
...
----   para $
----     \case
----       ELet (_, name) (Fix (ECall _ (t, fun) args), _) (_, body)
----         | arity t > length args ->
...
----     Just (Right (_, op)) ->
----       call op (zip as (repeat []))
----     Just (Left (fun, args0)) -> do
----       emitCall fun ((emitBody <$> args0) <> args)
----     _ ->
----       error "#3"
---- 
---- --        EVar (t, "add1") ->
...
llvmType =
  project
    >>> \case
      ...
      ty@TArr{} -> ptr funTy -- ptr (StructureType False [ptr funTy])
        where
