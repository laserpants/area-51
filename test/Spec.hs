...
hello3xx s = do
  ...
  Program i <- translate2by (Program h)
...
hello4x =
  case runTypeChecker 1 mempty hello3x of
    (Right p, (n, _)) -> do
      ...
      --traceShowM p
      --traceShowM "==="
      x <- runReaderT (translate3 p) mempty
      (_, z) <- get
      pure (x <> z)
    --traceShowM "=1="
    ----mapM_ traceShowM x
...
hello6xx s = do
  ...
  let Program qqq = qq
  mapM_ traceShowM (Map.toList qqq)
  traceShowM "^^^^^^^^^^^^^^^"
  runModule yy1
  x <- foo
  traceShowM x
    ...
    ( Map.fromList
        ...
        ,
          ...
          , Function
              ...
              , eLet
                  ((), "g")
                  (eApp () (eVar ((), "f")) [eLit (PInt 8)])
                  ( eIf
                      (eOp2 ((), OEq) (eApp () (eVar ((), "g")) [eLit (PInt 9)]) (eLit (PInt 9)))
                      (eLit (PInt 2))
                      (eLit (PInt 1))
                  )
              )
...
--   let g = f(8) in if g(9) == 9 then 2 else 1
--
card2 :: Program MonoType TypedExpr
card2 =
  Program
    ( Map.fromList
        [
        ...
        ,
          ...
          , Function
              ...
              , eLet
                  (tVar 9 ~> tInt, "g")
                  (eApp (tVar 9 ~> tInt) (eVar (tInt ~> tVar 9 ~> tInt, "f")) [eLit (PInt 8)])
                  ( eIf
  ...
  Program
    ( Map.fromList
        ...
        , ( (Scheme ((tInt ~> tInt ~> tInt) ~> tInt ~> tInt), "$lam2")
          , Function
              (fromList [(tInt ~> tInt ~> tInt, "f"), (tInt, "v")])
              ( tInt
              , eCall (tInt ~> tInt ~> tInt, "f") [eLit (PInt 8), eVar (tInt, "v")]
              )
          )
        , ( (Scheme (tUnit ~> tInt), "main")
          , Function
...
card5Test2 = do -- runModule (buildProgram "Foo" card5)
  --let Program qqq = qq
  runModule yy1
  x <- foo
  traceShowM x
          ...
          , Function
              ...
              , eLet
                  ...
                  ( eLet
                      ...
                      ( eIf
                          (eOp2 oEqInt (eCall ((tInt ~> tInt ~> tInt) ~> tInt ~> tInt, "$var_g_1") [eVar (tInt ~> tInt ~> tInt, "$var_f_1"), eLit (PInt 9)]) (eLit (PInt 9)))
                          (eLit (PInt 2))
                          (eLit (PInt 1))
                      )
                  )
